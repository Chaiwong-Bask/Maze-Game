...
public class SongVideoPlayer : MonoBehaviour
{
    ...
    {
        if (!hasLoadedVideo || IsWaitingForVideoGap(positionInSongInMillis) || nextSyncTimeInSeconds > UnityEngine.Time.time)
        {
            return;
        }

        // Both, the smooth sync and immediate sync need some time.
        nextSyncTimeInSeconds = UnityEngine.Time.time + 0.5f;
