...
public class FlightController : MonoBehaviour {
	...
	void Update () {
		...
		this.transform.position += direction.normalized * this.Velocity * UnityEngine.Time.deltaTime;

		this.Velocity = Mathf.Clamp(this.Velocity + Input.GetAxis("Move Y") * UnityEngine.Time.deltaTime * 4f, 0f, 20f);
		this.transform.rotation = Quaternion.Euler(Vector3.up * (Input.GetAxis("Look X") + Input.GetAxis("Mouse Look X")) * UnityEngine.Time.deltaTime * this.LookSensitivity) * this.transform.rotation;
	}
}
