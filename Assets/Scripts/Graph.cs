...
public class Graph : MonoBehaviour
{
	...
	/// </summary>
	[SerializeField]
	protected List<Node_D> m_Nodes = new List<Node_D> ();

	/// <summary>
	/// Gets the nodes.
	/// </summary>
	/// <value>The nodes.</value>
	public virtual List<Node_D> nodes
	{
		get
		{
			return m_Nodes;
	...
	public virtual Path_D GetShortestPath ( Node_D start, Node_D end )
	{
		...
		List<Node_D> unvisited = new List<Node_D> ();
		
		// Previous nodes in optimal path from source
		Dictionary<Node_D, Node_D> previous = new Dictionary<Node_D, Node_D> ();
		
		// The calculated distances, set all to Infinity at start, except the start Node
		Dictionary<Node_D, float> distances = new Dictionary<Node_D, float> ();
		
		for ( int i = 0; i < m_Nodes.Count; i++ )
		{
			Node_D node = m_Nodes [ i ];
			unvisited.Add ( node );
		...
		while ( unvisited.Count != 0 )
		{
			...
			{
				Node_D neighbor = current.connections [ i ];
